...
class TestTagApi(SupersetTestCase):
    ...
    def test_delete_favorite_tag(self):
        ...
        association_row = (
            db.session.query(user_favorite_tag_table)  # noqa: F821
            .filter(
                and_(  # noqa: F821
                    user_favorite_tag_table.c.tag_id == tag.id,  # noqa: F821
                    user_favorite_tag_table.c.user_id == user_id,  # noqa: F821
                )
            )
            .one_or_none()
        )
